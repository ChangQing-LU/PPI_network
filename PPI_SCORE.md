...
上表是核心 PPI scorer 的输出字段，也就是 `RareDiseasePPIScorer.run()` 或 `/score` 接口返回的 PPI 结果。clean-case 病例入口会在这些 PPI 字段之外，再合并 phenotype-gene CSV 和 VEP CSV 的字段，生成最终病例 CSV。

### Clean-case 最终 CSV 输出

`run_clean_case.py`、`/score/clean-case`、`/score/clean-case/async` 和 `/score/clean-case/upload/async` 生成的是最终病例表。使用 `output_all_ppi_fields=true` 时，当前最终 CSV 列顺序如下：
...
| `ppi_final` | PPI scorer | 最终 PPI 综合分数 |
| `in_network` | PPI scorer | 是否存在于 STRING 网络 |
| `disease_score` | PPI scorer | 与疾病锚点 `D` 的互作和距离分数 |
| `tissue_score` | PPI scorer | 与组织核心锚点 `T` 的互作和距离分数 |
| `topology_score` | PPI scorer | 全局 degree 和 betweenness 拓扑分数 |
| `score_mode` | PPI scorer | 本次融合模式，例如 `FULL_ANCHOR`、`NO_D_REWEIGHTED`、`TOPOLOGY_ONLY` |
| `score_weight_sum` | PPI scorer | 本次参与 PPI 融合的权重和 |
| `note` | PPI scorer | 边界情况标记，例如 `EMPTY_D`、`EMPTY_T`、`NOT_IN_STRING` |
| `gene_in_d` | PPI scorer | 该候选基因是否进入本次疾病锚点 `D` |
| `gene_d_evidence_score` | PPI scorer | 该候选基因在 D 集证据字典中的加权证据分；未命中为 0 |
| `gene_d_sources_json` | PPI scorer | 支持该候选基因进入 D 集的来源列表 JSON |
| `gene_in_t` | PPI scorer | 该候选基因是否进入本次组织核心锚点 `T` |
| `gene_t_weight` | PPI scorer | 该候选基因的 T 层支持权重；未命中为 0 |
| `gene_t_layers_json` | PPI scorer | 支持该候选基因进入 T 集的层来源 JSON |
| `gene_t_tissues_json` | PPI scorer | 支持该候选基因进入 T 集的组织列表 JSON |
| `mapped_tissues_json` | PPI scorer | 本次 HPO 解析得到并按映射次数排序取 Top 3 的组织列表 JSON |
| `d_gene_count` | PPI scorer | 本次最终 D 集基因数 |
| `t_gene_count` | PPI scorer | 本次最终 T 集基因数 |
| `top_neighbors_json` | PPI scorer | STRING 互作最强的 Top N 邻居及 D/T 标签 |
| `top_neighbors_count` | PPI scorer | 返回的邻居数量 |
| `gene_rank` | phenotype CSV | phenotype-gene 输入表中的基因排名 |
| `ppi_rank` | PPI scorer | 仅按 `ppi_final` 排序得到的 PPI 排名 |
...
| `mapped_tissues` | PPI scorer | 本次映射组织的逗号分隔字符串，便于表格查看 |
| `mapped_tissue_counts_json` | PPI scorer | 本次 HPO 到组织映射计数字典 JSON |
...
`final_rank` 只表示可融合基因的综合排名。某个基因如果有 `ppi_final` 但 phenotype 输入中的 `gene_score` 为空，最终 CSV 会保留该基因及其 PPI 字段，但 `combined_score` 和 `final_rank` 会为空。`ppi_rank` 可用于查看只按 PPI 分数排序的名次。
